...
get_M = function(data){
  ...
  for(j in 1:p){
    ...
    X <- data[, -j]
    if(j == 1){
      cvfit <- cv.glmnet(X, y, type.measure = "mse", nfolds = 10, nlambda = 200, intercept = F, standardize = F)
...
save(data_save, file = paste("~/result_left/signal_", signal, "_replicate_", replicate, ".RData", sep = ""))
